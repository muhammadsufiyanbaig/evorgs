...
import type { CateringCustomPackage } from "@/utils/interfaces"
import { mockUsers } from "@/utils/data"

interface CateringRequestsTableProps {
  requests: CateringCustomPackage[]
}

export function CateringRequestsTable({ requests }: CateringRequestsTableProps) {
  const [selectedRequest, setSelectedRequest] = useState<CateringCustomPackage | null>(null)
  const [isQuoteModalOpen, setIsQuoteModalOpen] = useState(false)
  const [quotePrice, setQuotePrice] = useState("")

  const handleOpenQuoteModal = (request: CateringCustomPackage) => {
  ...
  const handleQuoteSubmit = () => {
    ...
    // In a real app, this would trigger an API call to update the request
  ...
  const getStatusBadge = (status: CateringCustomPackage["status"]) => {
    const map = {
  ...
  return (
    <>
      <Card className="border-orange-200 bg-white">
        <CardHeader>
          <CardTitle>Custom Catering Packages</CardTitle>
          <CardDescription>Requests for custom catering services from users.</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <DialogContent>
            ...
            <div className="py-4 space-y-4">
              ...
              <div>
                ...
                <p className="text-sm">{selectedRequest.guestCount}</p>
              </div>
